defmodule Agento.IdentificationContextTest do
  use Agento.DataCase
  ...
  describe "identification" do
    ...
    def identifications_fixture(attrs \\ %{}) do
      {:ok, identifications} =
        ...
        |> IdentificationContext.create_identifications()
    ...
    test "list_identification/0 returns all identification" do
      ...
      assert IdentificationContext.list_identification() == [identifications]
    end

    test "get_identifications!/1 returns the identifications with given id" do
      identifications = identifications_fixture()
      assert IdentificationContext.get_identifications!(identifications.id) == identifications
    end

    test "create_identifications/1 with valid data creates a identifications" do
      assert {:ok, %Identifications{} = identifications} = IdentificationContext.create_identifications(@valid_attrs)
    ...
    test "create_identifications/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = IdentificationContext.create_identifications(@invalid_attrs)
    end

    test "update_identifications/2 with valid data updates the identifications" do
      identifications = identifications_fixture()
      assert {:ok, %Identifications{} = identifications} = IdentificationContext.update_identifications(identifications, @update_attrs)
    ...
    test "update_identifications/2 with invalid data returns error changeset" do
      identifications = identifications_fixture()
      assert {:error, %Ecto.Changeset{}} = IdentificationContext.update_identifications(identifications, @invalid_attrs)
      assert identifications == IdentificationContext.get_identifications!(identifications.id)
    end

    test "delete_identifications/1 deletes the identifications" do
      identifications = identifications_fixture()
      assert {:ok, %Identifications{}} = IdentificationContext.delete_identifications(identifications)
      assert_raise Ecto.NoResultsError, fn -> IdentificationContext.get_identifications!(identifications.id) end
    end

    test "change_identifications/1 returns a identifications changeset" do
      identifications = identifications_fixture()
      assert %Ecto.Changeset{} = IdentificationContext.change_identifications(identifications)
    end
  end
end
